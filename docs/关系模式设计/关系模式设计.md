...
| 存储过程名 | 说明 |
|------------|------|
| `sp_update_query_statistics(p_date)` | 使用游标遍历指定日期的查询记录，按类别分组统计并更新 `query_statistics_cache` 表 |
| `sp_get_personalized_recommendations(p_user_id, p_limit)` | 基于用户历史查询偏好，使用临时表计算偏好类别，推荐相关近期活动 |

### 7.3 触发器（Triggers）

| 触发器名 | 说明 |
|----------|------|
| `trg_after_query_insert` | 每次向 `query_record` 插入新记录后，自动更新 `query_statistics_cache` 当天的统计数据 |

### 7.4 事件调度器（Events）

| 事件名 | 调度周期 | 说明 |
|--------|----------|------|
| `evt_daily_statistics_update` | 每天凌晨 00:00 | 自动调用 `sp_update_query_statistics` 更新前一天的统计数据 |
